export const executeShortcut = (e) => {
    ...
    if (e.key === 'z') {
        const keyboardInstructions = document.querySelector('details');
        keyboardInstructions.open === false ? keyboardInstructions.open = true : keyboardInstructions.open = false;
    }
    if (e.key === 'x') document.getElementById('dark-mode').click();
    if (e.key === 'c') document.getElementById('animation-mode').click();
}